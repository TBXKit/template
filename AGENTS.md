...
Components are grouped by Tebex domain where a domain has more than one related component — `components/category/` (`category-card.tsx`, `category-grid.tsx`, `category-detail.tsx`), `components/package/` (`package-card.tsx`, `package-detail.tsx`, `package-price.tsx`, `package-badge.tsx`, ...), and `components/basket/` (`basket-item.tsx`, `basket-summary.tsx`, `basket-empty.tsx`, `remove-from-basket-button.tsx`). Components without siblings stay as flat files at the top of `components/` (`header.tsx`, `footer.tsx`, `hero.tsx`, `breadcrumbs.tsx`, `store-disabled-banner.tsx`).
