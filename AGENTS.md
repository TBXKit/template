...
- **`mapWebstore`/`mapCategory`/`mapPackage` treat their input as fully untrusted (`unknown`), not just "optional per the generated schema."** They validate every field by *type*, not just presence — a numeric ID sent as a string, a price that's `NaN`/`Infinity`, or a boolean sent as `"yes"` all fall back to the same default a missing field would get, rather than passing the wrong type through to the domain object. None of the three ever throws: a malformed top-level argument (`null`, a primitive, an array) returns a fully-defaulted domain object instead of crashing the caller. Malformed entries *inside* an array (a `null` in `Category.packages`, a string in `Package.media`) are dropped rather than defaulted, so one bad entry can't take down the rest of the list — but an empty object (`{}`) is still kept and mapped to a fully-defaulted placeholder, consistent with how a fully-empty top-level response is handled. The mapper deliberately does not coerce types (a numeric string is treated as invalid, not parsed into a number) and does not enforce business rules (a negative price passes through unchanged) — only shape/type safety is its job.
